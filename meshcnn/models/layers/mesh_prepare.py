...
def from_scratch(file, opt=None, requires_feature=False):
    ...
    if requires_feature:
        mesh_data.features = extract_features(mesh_data)   # We don't need MeshCNN feature in our setting
    else:
...
def fill_from_file(mesh, file):
    if isinstance(file, str):
        ...
        vs, faces = file
        if isinstance(vs, torch.Tensor): vs = vs.detach().cpu().clone().numpy()
        if isinstance(faces, torch.Tensor): faces = faces.detach().cpu().clone().numpy()
    return vs, faces
